...
for x in range(w):
    for y in range(h):
        mat[x, y] = (x**2-y**2) % 256  # (x**2 + y**2) % 256
# img1.show()


# Create mask image from our cow
mask = Image.new("RGB", (w, h), "white")
draw = ImageDraw.Draw(mask)
draw.polygon(first, fill="rgb(0, 0, 0)", outline="black")
#draw.polygon(second, fill="rgb(0, 0, 0)", outline="black")
mask = mask.convert('L')


# Create the composite image
img3 = Image.composite(img, img1, mask)

# Add text to image
draw2 = ImageDraw.Draw(img3)
font = ImageFont.truetype('arial', 30)
font2 = ImageFont.truetype('arial', 40)
draw2.text((20, 10), 'Many thanks to all of the next.py team',
           fill='red', font=font)
draw2.text((350, 400), 'TadyTiodros', fill='red', font=font2)
# img3.show()

# save the image
img3.save("cow.PNG", "PNG")
